...
qq.FileUploaderBasic = function(o){
    ...
    this._options = {
		...
		customErrorTips: function(item,result){}, // 自定义错误提示，2013-07-18增加
        // messages
        messages: {
            typeError: "{file} has an invalid extension. Valid extension(s): {extensions}.", // "{file} 格式不对。 允许上传的文件格式为： {extensions}."
            sizeError: "{file} is too large, maximum file size is {sizeLimit}.", //"{file} 太大了， 允许上传的文件大小最大为： {sizeLimit}."
            minSizeError: "{file} is too small, minimum file size is {minSizeLimit}.", //"{file} 太小了，允许上传的文件大小最小为： {minSizeLimit}."
            emptyError: "{file} is empty, please select files again without it.", //"{file} 是空的（可能为快捷方式），请重新选择有效文件。"
            noFilesError: "No files to upload.", //"没有可上传文件。"
            onLeave: "The files are being uploaded, if you leave now the upload will be cancelled." //"文件上传中，可点击“取消”中断。"
        },
...
qq.extend(qq.FileUploader.prototype, {
    ...
    _onComplete: function(id, fileName, result){
        ...
        if (result.success){
			...
			this._options.customErrorTips(item,result);
